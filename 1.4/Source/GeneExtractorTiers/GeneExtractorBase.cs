...
namespace GeneExtractorTiers
{
    ...
    public abstract class Building_GeneExtractorTier : Building_Enterable, IStoreSettingsParent, IThingHolderWithDrawnPawn, IThingHolder
    {
        ...
        public int ExtractionTimeInTicks => (int)(Settings.extractionHours * 2500 / SpeedMultiplier);

        private const float WorkingPowerUsageFactor = 5.5f;
        ...
        private void Finish()
        {
            if (GetContainedPawn() != null)
            {
                ...
                // Check if baseliner
                if (// Get baseliner-like xenotypes. (e.g. only skin and hair genes)
                    pickableGenes.All(x=>x.defName.ToLower().Contains("skin") || x.defName.ToLower().Contains("hair")) || pickableGenes.Count == 0)
                {
                ...
                genepack.Initialize(genesInPack);
